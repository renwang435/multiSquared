...
from utils import FUSION_LAYERS, apply_3d_transformation
...
class PointFusion(nn.Module):
    ...
    def forward(self, img_feats, pts, pts_feats, img_metas):
        ...
        # sys.exit(1)
        pts_pre_fuse = self.pts_transform(pts_feats_flat)
    ...
    def obtain_mlvl_feats(self, img_feats, pts, img_metas):
        ...
        # print(len(img_feats_per_point))
        # print(mlvl_img_feats.shape)
        # sys.exit(1)

        img_pts = torch.cat(img_feats_per_point, dim=0)
        # img_pts = torch.stack(img_feats_per_point, dim=0)
        return img_pts
